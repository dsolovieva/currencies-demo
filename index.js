...
app.get('/dailyQuotes', function(req, res){
    var date = new Date();
    restler.request(dailyQuotesUrl, {query: {date_req: date.getDate() + "/" + date.getMonth()+"/" + date.getFullYear()}, encoding: 'windows-1251',
        parser: restler.parsers.xml}).on('complete', function(result) {
...
app.post('/quoteDynamic', function(req, res){
    ...
    for (var i = 0; i<graphParamList.length; i++){
        ...
        restler.request(quoteDynamicUrl, {query: {date_req1: dateStart.getDate() + "/" + dateStart.getMonth()+"/" + dateStart.getFullYear(),
            date_req2: dateEnd.getDate() + "/" + dateEnd.getMonth() + "/" + dateEnd.getFullYear(), VAL_NM_RQ: code}, encoding: 'windows-1251',
            parser: restler.parsers.xml}).on('complete', function(result) {
            resCount++;
            if (result instanceof Error) {
                console.log('Error:', result.message);
